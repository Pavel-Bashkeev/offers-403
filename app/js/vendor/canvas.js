...
const config = {
  curvesNum    : 3,
  waveSpeed    : 1,
  wavesToBlend : 3,  //waves count to blend 
  framesToMove : 60 //frames count to change type placement
}
...
class Animation {
  ...
  createCanvas() {
    ...
    this.setCanvasSize();
    document.querySelector('.footer').appendChild(this.cnv);
    window.addEventListener(`resize`, () => this.setCanvasSize());
  }
  setCanvasSize() {
    this.size.w  = this.cnv.width  =  document.querySelector('.footer').clientWidth;
    this.size.h  = this.cnv.height =  document.querySelector('.footer').clientHeight;
  ...
  drawCurve({startX, startY, controlX1, controlY1, controlX2, controlY2, endX, endY, alpha}) {
    ...
    this.ctx.lineTo(endX, startY * 2);
      this.ctx.lineTo(0, startY * 2);
